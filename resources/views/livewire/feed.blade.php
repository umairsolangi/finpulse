<div class="py-8 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 fp-animate-in">
        <div>
            ...
            <p class="text-sm text-finpulse-gray mt-1">Discuss financial insights, stocks, mutual funds, and market news.</p>
        </div>
    </div>
    ...
    @if(auth()->user()?->can('post.create'))
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 sm:p-6 space-y-4 fp-animate-in">
            ...
            <form wire:submit="createPost" class="space-y-4">
                <div>
                    <label for="category" class="block text-xs font-semibold text-finpulse-gray uppercase tracking-wider mb-1">
                        Category
                    </label>
                    <select
                        id="category"
                ...
                <div>
                    ...
                    <textarea
                        ...
                        wire:model="body"
                        rows="3"
                        placeholder="What's on your financial radar today?"
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-2 border-b border-gray-200 fp-animate-in">
        ...
        @foreach($categories as $cat)
            <button
                type="button"
                wire:click="selectCategory('{{ $cat->value }}')"
    ...
    <div class="space-y-4 fp-stagger-grid">
        @forelse($posts as $post)
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 sm:p-6 space-y-4 hover:shadow-lg hover:border-gray-300 transition-all duration-300 group">
                <!-- Post Header -->
                <div class="flex items-start justify-between gap-3">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-finpulse-navy text-white flex items-center justify-center font-bold text-base shrink-0 shadow-sm group-hover:scale-110 transition-transform duration-300">
                            {{ strtoupper(substr($post->user->name ?? 'U', 0, 1)) }}
                        </div>
                        <div>
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="font-semibold text-gray-900 text-sm sm:text-base">{{ $post->user->name ?? 'User' }}</span>
                ...
                <div class="pt-2 border-t border-gray-100 flex items-center gap-6">
                    ...
                    <button
                        type="button"
                        wire:click="toggleReaction({{ $post->id }})"
                        ...
                        title="React to post"
                    >
                        <svg class="w-4 h-4 {{ $hasReacted ? 'fill-current text-red-600' : 'fill-none stroke-current' }} {{ $hasReacted ? 'fp-react-pop' : '' }}" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 016.364 0L12 7.293l.318-.318a4.5 4.5 0 116.364 6.364L12 21.364l-7.682-7.682a4.5 4.5 0 010-6.364z" />
                        </svg>
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100 space-y-3 bg-gray-50/50 rounded-lg p-4 fp-page-enter">
                        ...
                        <div class="space-y-3">
                            @forelse($post->comments as $comment)
                                <div class="bg-white rounded-lg p-3 border border-gray-200 text-xs sm:text-sm space-y-1 hover:shadow-sm transition-shadow">
                                        ...
                                        <div class="flex items-center gap-2 flex-wrap">
                                            ...
                                            @if($comment->user && $comment->user->badges->isNotEmpty())
                                                @foreach($comment->user->badges as $badge)
                                                    <span class="inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.2 rounded-full bg-amber-100 text-amber-900 border border-amber-200">
                                                        🏆 {{ $badge->name }}
                                                    </span>
                                                @endforeach
                                            @endif
